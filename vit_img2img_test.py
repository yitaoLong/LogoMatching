...
#img_files=glob('sample/*.png', recursive=True)
#images2 = [cv2.resize(i, (32, 32)) for i in map(cv2.imread, img_files[:2]) if i is not None]
## transpose the images because they're stored in a weird color channel first format, as indicated by shape[0] being 3
#images2  = [np.transpose(i) if i.shape[0] == 3 else i for i in images2]
#
#images.extend(images2)
...
run_in_chunks5(methods, images, aberrations, feature_extractor, model, chunk_size=100, weights=[],
    logdir=logdir)
print("Done")
